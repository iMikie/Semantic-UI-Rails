...
This **README** describes step-by-step how to add Semantic-UI to a new Rails app, then we'll build the UI for the example shown below.  In doing so we'll take advantage of Semantic-UI's support for mobile responsive design and (surprisingly elegant support for) client side validation.  If you find you need more help with Semantic-UI itself, Lawrence Turton has a nice non-rails tutorial over on [Tuts](https://webdesign.tutsplus.com/courses/getting-started-with-semantic-ui).  
